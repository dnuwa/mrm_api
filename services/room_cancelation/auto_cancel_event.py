    ...
    def update_recurring_event_status(self):
        ...
        for event in events:
            ...
            if missed_checkins.count() >= 3:
                ...
                organizer, room_id, event_title = (
                    event_in_database.organizer, event_in_database.room_id,
                    event_in_database.event_title
                )
                event_reject_reason = "for 3 consecutive meetings"
                if not event_cancellation_notification(
                    organizer, room_id, event_title, event_reject_reason
                ):
